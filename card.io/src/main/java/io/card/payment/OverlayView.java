...
class OverlayView extends View {
    ...
    private static final int MANUAL_MAX_WIDTH = 400;
    private static final int LOGO_MAX_HEIGHT = 100;
    ...
    private final ManualButton mManual; // new
    private Rect mTorchRect, mLogoRect, mManualRect;
    ...
    public OverlayView(CardIOActivity captureActivity, AttributeSet attributeSet, boolean showTorch) {
        ...
        mManual = new ManualButton(captureActivity); // new
    ...
    public void setGuideAndRotation(Rect rect, int rotation) {
        ...
        if (mCameraPreviewRect != null) {
            ...
            Point torchPoint = new Point(mCameraPreviewRect.centerX(),
                    mCameraPreviewRect.top - (topEdgeUIOffset.y));

            // mTorchRect used only for touch lookup, not layout
            mTorchRect = Util.rectGivenCenter(torchPoint, (int) (TORCH_WIDTH * 3),
                    (int) (TORCH_HEIGHT * 5));

            // mLogoRect used only for touch lookup, not layout
            Point logoPoint = new Point(mCameraPreviewRect.centerX(),
                    mCameraPreviewRect.bottom - (topEdgeUIOffset.y)*2);
            mLogoRect = Util.rectGivenCenter(logoPoint, (int) (LOGO_MAX_WIDTH * 3),
                    (int) (LOGO_MAX_HEIGHT * 10));

            mManualRect = Util.rectGivenCenter(logoPoint, (int) (MANUAL_MAX_WIDTH * mScale),
...
//        }
         ...
         canvas.translate(mManualRect.exactCenterX(), mManualRect.exactCenterY()); //location
         mManual.draw(canvas, MANUAL_MAX_WIDTH * mScale, LOGO_MAX_HEIGHT * mScale);
...
//                    mScanActivityRef.get().triggerNextActivity();
    ...
    public void setUseCardIOManualButton(boolean useCardIOManualButton) { // new
        mManual.loadManualButton(useCardIOManualButton);
    }
